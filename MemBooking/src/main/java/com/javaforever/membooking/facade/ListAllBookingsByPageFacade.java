...
public class ListAllBookingsByPageFacade extends HttpServlet{
	...
	protected void processRequest(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
		...
		try {
			...
			int pagecount = (Integer)service.countBookingsPage(pagesize);
			if (pagenum <= 1) pagenum = 1;
			if (pagenum >= pagecount) pagenum = pagecount;
			if (last) pagenum = pagecount;
			List<Booking> bookingList = service.listAllBookingsByPage(pagesize,pagenum);
			result.put("success",true);
			result.put("rows",bookingList);
